...
int main(int argc, char **argv) {
    ...
    double turn_speed = 0.5*pi/3; //quater of circle in 3 seconds (in length)
    double yaw_rate_fixed = 0.5*pi/3; //quater of circle in 3 seconds (in angle)
    double time_1_sec = 1.0;//a time standard
    ...
    twist_cmd.angular.z=yaw_rate_fixed; //and start spinning in place
    ...
    //start editing following motions
    twist_cmd.linear.x=0.0; 
    twist_cmd.angular.z=-yaw_rate_fixed; 
    timer=0.0; 
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
